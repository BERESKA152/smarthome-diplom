...
function initCart() {
    ...
    btnsAdd.forEach(btn => {
        btn.addEventListener('click', e => {
            ...
            const price = priceText.replace('BYN ', '')
...
function getTotal() {
    document.querySelector('#len-cart').textContent = products.reduce((a,b) => a + b.quantity, 0)
    const sum = products.reduce((sum, product) => sum + ( product.price * product.quantity), 0);




    
    document.querySelector('[data-total-sum]').innerHTML = sum
}
